...
public class MobileMoneyTransactionService {
        ...
        if (inputParts.length > 1) {
            ...
            if (pin.matches("\\d{4}")) {
                ...
                if ("0".equals(responseDto.getStatus())) {
                    ...
                    // TODO: Add prompt for depositorId
    ...
    public String handleWithdrawPinEntry(UssdSession session, String[] inputParts, String apiKey) throws IOException, IOException {
        if (inputParts.length > 1) {
            String pin = inputParts[inputParts.length - 1];
            if (pin.matches("\\d{4}")) {
                session.setPin(pin);
